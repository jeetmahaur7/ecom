        ...
        bcrypt.hash(req.body.password, 12, async (error, hash) => {
            if (error)
                ...
                try {
                    ...
                    jwt.sign({ data }, process.env.JWT_SECERT_KEY_BUYER, (error, token) => {
                        if (error)
...
async function login(req, res) {
    try {
        ...
        if (data && await bcrypt.compare(req.body.password, data.password)) {
            let secretKey = data.role === "Buyer" ? process.env.JWT_SECERT_KEY_BUYER : process.env.JWT_SECERT_KEY_ADMIN
